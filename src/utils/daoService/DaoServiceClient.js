...
(function(window) {
  ...
  DaoServiceClient.prototype = /** @lends DaoServiceClient.prototype */ {
    ...
    config: function(cfg) {
      ...
      this.projectId = require(cfg.projectKey, '请设置您要请求的工程名称: projectKey');
    ...
    executeCustom: function(requestBody, callback) {
      var url = getCustomApiUrl(requestBody);
      post(url, requestBody, callback);
    },
    forImport: function() {
  ...
  function getCustomExportUrl(req) {
    var client = ds.client;
    return (
      client.serverUrl +
  ...
  function getDownloadFileUrl(req) {
    var client = ds.client;
    return (
      client.serverUrl +
  ...
  ModelCommand.prototype = /** @lends ModelCommand.prototype */ {
    ...
    update: function() {
      return new UpdateCommand(this, toFields(arguments));
    },
    updateSelective: function() {
      return new UpdateSelectiveCommand(this);
    },
    updateBatch: function() {
      return new UpdateBatchCommand(this);
    },
    addBatch: function() {
   ...
   * @constructor
   */
  function UpdateCommand(modelCommand, fieldExpressions) {
    this.modelCommand = modelCommand;
    this.fieldExpressions = fieldExpressions;
    this.definedArguments = {};
  }
  ...
  extend(UpdateCommand.prototype, {
    ...
    execute: function() {
      var extractArgs = extractExecuteArguments(arguments);
      var me = this,
        model = me.modelCommand.modelExpression,
        isUpdateSpecifyFields = !isEmpty(this.fieldExpressions),
        requestArguments = {},
        whereExpression = this.whereExpression,
        argArray = extractArgs.executeArgs,
        fieldExpression,
        i,
        set,
        argumentObject;
      if (isEmpty(argArray)) {
        throw new Error('更新命令参数不能为空');
      }
      if (isUpdateSpecifyFields) {
        argumentObject = argArray[0];
        if (isArrayLikeArguments(argumentObject)) {
          for (i = 0; i < this.fieldExpressions.length; i++) {
            fieldExpression = this.fieldExpressions[i];
            requestArguments[fieldExpression.getFinalFieldName()] = argumentObject[i];
          }
          if (whereExpression != null) {
            whereExpression.accept(
              new DeepExpressionVisitor(function(expression) {
                if (expression instanceof ExplicitValueParameter) {
                  requestArguments[expression.argumentName] = expression.argumentValue;
                }
                if (expression instanceof NamedParameter) {
                  requestArguments[expression.argumentName] = argumentObject[i];
                  i += 1;
                }
              }),
            );
          ...
          for (i = 0; i < this.fieldExpressions.length; i++) {
            fieldExpression = this.fieldExpressions[i];
            requestArguments[fieldExpression.getFinalFieldName()] =
              argumentObject[fieldExpression.getFinalFieldName()];
          }
          if (whereExpression != null) {
            whereExpression.accept(
              new DeepExpressionVisitor(function(expression) {
                ...
                if (expression instanceof NamedParameter) {
                  requestArguments[expression.argumentName] =
                    argumentObject[expression.argumentName];
                }
              }),
            );
          }
        }
        set = map(this.fieldExpressions, function(field) {
          var fieldName = field.fieldName;
          return fieldName + ' = :' + fieldName;
        });
      } else {
        argumentObject = requestArguments = argArray[0];
        if (me.updateDeepTree) {
          var knownFieldNames = map(model._allFields, function(field) {
            return field.fieldName;
          });
          requestArguments = pick(argumentObject, knownFieldNames);
        }
      }
      var requestBody = {
        crudType: 'UPDATE',
        action: 'update',
        mainModelName: model._modelName,
        tag: me.tagName,
        where: me.whereExpression && me.whereExpression.hqlFragment(),
        reqArgs: requestArguments,
      };
      if (isUpdateSpecifyFields) {
        requestBody.action = 'updateWhere';
        requestBody.set = set;
      }
      if (!!requestBody.where) {
        requestBody.action = 'updateWhere';
      }
      return me.modelCommand.client.execute(requestBody, extractArgs.callback);
    },
   ...
   * @constructor
   */
  function UpdateSelectiveCommand(modelCommand) {
    this.modelCommand = modelCommand;
  }
  ...
  extend(UpdateSelectiveCommand.prototype, {
    execute: function(argumentObject, callback) {
      var me = this,
      ...
      var pkValue = argumentObject[pkName];
      if (!pkValue) {
      ...
      var valueNotnullFields = filter(withoutPrimaryKeyFields, function(field) {
        return !!argumentObject[field.fieldName];
      });

      return this.modelCommand
        .update(new BasicFieldContainer(valueNotnullFields))
        .where(modelExpression.primaryKey().eq$(pkValue))
        .tag(this.tagName)
        .execute(argumentObject, callback || noop);
    },
  });
   ...
   * @constructor
   */
  function UpdateBatchCommand(modelCommand) {
    this.modelCommand = modelCommand;
  }

  UpdateBatchCommand.prototype = new BaseCommand();

  extend(UpdateBatchCommand.prototype, {
    execute: function() {
      var extractArgs = extractExecuteArguments(arguments),
        me = this,
        model = me.modelCommand.modelExpression,
        requestArguments = extractArgs.executeArgs[0];
      var requestBody = {
        crudType: 'UPDATE',
        action: 'update',
        mainModelName: model._modelName,
    ...
    extractRequestArguments: function(args) {
      var me = this,
        model = me.modelCommand.modelExpression;
      if (me.updateDeepTree) {
        var knownFieldNames = map(model._allFields, function(field) {
        ...
        if (isArray(args)) {
          return map(args, function(obj) {
            return pick(obj, knownFieldNames);
          });
        } else {
          return pick(args, knownFieldNames);
        }
      }
      return args;
    },
  ...
  AddBatchCommand.prototype.execute = function() {
    ...
    return me.modelCommand.client.execute(requestArguments, extractArgs.callback);
  };
  ...
  DeleteCommand.prototype.id = function() {
    var primaryKey = this.modelCommand.modelExpression.primaryKey();
    var whereExpression = primaryKey.in$(flatten(toArray(arguments)));
  ...
  DeleteCommand.prototype.execute = function() {
    ...
    var me = this;
    if (!me.whereExpression) {
      throw new Error('执行删除时where条件不能为空');
    }
    ...
    return me.modelCommand.client.execute(reqArgObj, extractArgs.callback);
  };
  ...
  ImportCommand.prototype.execute = function(successCallback, errorCallback) {
    ...
    var me = this;
    var reqArgs = {
      action: getImportAction(this.tagName),
  ...
  /**
   *
   * @param origin {BasicFieldContainer/FilteredFieldContainer}
   * @param predicate
   * @constructor
   */
  ...
  FilteredFieldContainer.prototype = {
    getFinalFieldExpression: function() {
      return filter(this.origin.getFinalFieldExpression(), this.predicate.constructor);
    },
    hqlFragment: function() {
      return filter(this.origin.getFinalFieldExpression(), this.predicate.constructor).map(
  ...
  function OperatorExpression() {
    var me = this;
    var pairFunctionName;
    foreach(ConditionTypes, function(conditionType) {
      pairFunctionName = conditionType.pairFunctionName;

      me[pairFunctionName] = (function(conditionType) {
        return function() {
          var me = this;
      ...
      me[pairFunctionName + '$'] = (function(pairFunctionName) {
        return function() {
    ...
    foreach(SupportedFunctions, function(supportFunction) {
      me[supportFunction.name] = function() {
  ...
  BetweenExpression.prototype = new OperatorExpression();
  extend(
  ...
  CombinableExpression.prototype = {
    or: function(right) {
  ...
  ds.types.UpdateBatchCommand = UpdateBatchCommand;
